...
void NBA::simSeason(Team *team1)
{
    ...
    {
        cout << team1->teamPlayers.at(i)->first_name << " averaged " << team1->teamPlayers.at(i)->avgPoints << endl;
    }
}

void NBA::simSeason()
{
    std::vector<Team *> play82;

    for (int i = 0; i < getTeams().size() - 1; i++)
    {
        bool found = false;
        while (getTeams().at(i)->totalGames < 82)
        // for (int k = 0; k < play82.size(); k++)
        // {
        //     if (getTeams().at(i)->teamName == play82.at(k)->teamName)
        //     {
        //         found = true;
        //     }
        // }
        // while (!found)
        {
            ...
            if (team2 != getTeams().at(i) && team2->totalGames < 82)
            {
                ...
                team2->score = 0;
                // if (getTeams().at(i)->totalGames == 82)
                // {
                //     play82.push_back(getTeams().at(i));
                // }
                // if (team2->totalGames == 82)
                // {
                //     play82.push_back(team2);
                // } //^^^
            }
            else
            {
...
void NBA::playGame(Team *team1, Team *team2)
{
    ...
    std::cout << team1->teamName << " scored " << team1->score << " points " << endl
              << endl;
    ...
    std::cout << team2->teamName << " scored " << team2->score << " points " << endl
              << endl;
